...
//-------------------------------------------------------------------------------
void CaptureEx::CaptureDesktop (double fPreviewRatio,LPTSTR lpszFileName,BOOL bSaveToFile,int nSize)
{
	fPreviewRatio;
	RECT rc;
	// Capture appointed area.
	CPoint pt;
	GetCursorPos(&pt);
	SetRect(&rc, pt.x-nSize, pt.y-nSize, pt.x+nSize, pt.y+nSize);
//	HWND hWnd = ::GetDesktopWindow();
//	::GetWindowRect (hWnd,&rc); // Capture all screen.
...
void CaptureEx::StretchDIBitsToHwnd( HWND hWnd,double fRatio,int Width,int Height,int Bpp,BYTE *lpBits)
{
    ...
    bmp.bmiHeader.biBitCount = (WORD)Bpp;
	...
	double fOrg = (fScaleHeight-Height)/2;
//	::SetWindowPos(hWnd,0,rect.right,rect.top,50,50,SWP_SHOWWINDOW);
